...
//   { value: "uninstalled", label: "Uninstalled At" },
...
//               <TableHead>Uninstalled</TableHead>
...
function StoreTable({ stores, showUninstalled, showLastActivity }) {
  // Calculate column count dynamically
  const columnCount =
    8 + (showUninstalled ? 1 : 0) + (showLastActivity ? 1 : 0);
  ...
  return stores.filter((store) => {
    return filters.every((filter) => {
      switch (filter.type) {
        ...
        case "uninstalled":
          if (!store.uninstalledAt) return false;
          const uninstalledAt = new Date(store.uninstalledAt);
          return (
            (!filter.value.from ||
              uninstalledAt >= new Date(filter.value.from)) &&
            (!filter.value.to || uninstalledAt <= new Date(filter.value.to))
          );
        case "activity":
          const updatedAt = new Date(store.updatedAt);
          return (
